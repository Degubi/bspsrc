...
public final class MainGUI extends JFrame {
    ...
    public MainGUI() {
        ...
        FileDrop.attach(listFiles, files -> {
		    java.io.FileFilter filter = new SuffixFileFilter(".bsp", IOCase.INSENSITIVE);
    ...
    public ComboBoxModel getAppIDModel() {
        ...
        apps.stream()
            .sorted((SourceApp a1, SourceApp a2) -> a1.name.compareTo(a2.name))
    ...
    private File[] openBspFileDialog(File bspFile) {
        return openFileDialog(bspFile,
                new FileNameExtensionFilter("Source engine map files (*.bsp)", "bsp"));
    }

    private File saveVmfFileDialog(File vmfFile) {
        return saveFileDialog(vmfFile,
                new FileNameExtensionFilter("Hammer map files (*.vmf)", "vmf"));
    }
    ...
    private void initComponentsCustom() {
        ...
        setTitle("BSPSource " + Main.VERSION);
    ...
    private void initComponents() {
        ...
        radioButtonBrushesPlanes.addActionListener(evt -> config.brushMode = BrushMode.BRUSHPLANES);
        ...
        radioButtonSplitFaces.addActionListener(evt -> config.brushMode = BrushMode.SPLITFACE);
        ...
        radioButtonOrigSplitFaces.addActionListener(evt -> config.brushMode = BrushMode.ORIGFACE_PLUS);
        ...
        checkBoxCameras.addActionListener(evt -> config.writeCameras = checkBoxCameras.isSelected());
